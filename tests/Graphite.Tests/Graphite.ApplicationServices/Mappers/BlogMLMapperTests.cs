...
namespace Tests.Graphite.ApplicationServices.Mappers{
	...
	public class when_blogmlpost_is_mapped_to_post{
		[SetUp]
		public void Setup() { TestData.Init(); }
	}
	...
	public class when_post_is_mapped_to_blogmlpost{
		...
		[Test]
		public void post_author_is_mapped_by_emailaddress() { Assert.That(_mlPost.Authors[0].Ref == TestData.PublishedPost.Author.Email); }

		[Test]
		public void post_tags_are_mapped_to_categories() { Assert.That(_mlPost.Categories[0].Ref == TestData.PublishedPost.Tags[0].Name); }
	}
}